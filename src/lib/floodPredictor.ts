...
// Light geocoding for common place names — returns a plausible coordinate.
// Replace with a real geocoder (Google Places, OpenStreetMap Nominatim) later.
const PLACE_DB: Record<string, { lat: number; lng: number }> = {
...
export function geocode(query: string): GeoPoint | null {
  ...
  // Known place
  for (const key of Object.keys(PLACE_DB)) {
    if (q.includes(key)) {
      const p = PLACE_DB[key];
      return { lat: p.lat, lng: p.lng, label: query.trim() };
    }
  }

  // Fallback: deterministic pseudo-coordinate from string so any input works
  let seed = 0;
  for (let i = 0; i < q.length; i++) seed = (seed * 31 + q.charCodeAt(i)) % 100000;
  const r = seed / 100000;
  const lat = +(r * 50 + 8).toFixed(4);
  const lng = +((r * 360 + 20) % 360 - 180).toFixed(4);
